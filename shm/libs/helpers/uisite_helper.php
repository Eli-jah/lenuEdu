...
if (!function_exists('tag_single')) {
    function tag_photo($id, $column = 'url')
    {
        ...
        if (isset($a[$id][$column])) {
            return UPLOAD_URL . $a[$id][$column];
        }
        return '';
    }

    function first_photo($photo)
    {
        if (empty($photo)) {
            $photo = '';
        } else {
            if (strstr($photo, ',')) {
                ...
                $photo = tag_photo($photo_set[0]);
            } else {
                $photo = tag_photo($photo);
            }
        }
        return $photo;
    }

    function multi_photo($photo)
    {
        $photo_set = array();
        if (!empty($photo)) {
            if (strstr($photo, ',')) {
                $image_set = array();
                $image_set = explode(",", $photo);
                foreach ($image_set as $k => $v) {
                    $image = tag_photo($v);
                    $photo_set[] = $image;
                }
            } else {
                $image = tag_photo($photo);
                $photo_set['0'] = $image;
            }
        }
        return $photo_set;
...
if (!function_exists('tag_coltypes')) {
    function tag_coltypes($id, $column = null)
    {
        $CI =& get_instance();
        $CI->load->database();
        $data = $CI->db->order_by('sort_id', 'asc')->get_where('coltypes', array('id' => $id))->row_array();
        if (!empty($data)) {
            if (!empty($column)) {
                return $data[$column];
            } else {
...
function get_dealer($where)
{
    ...
    $sql = "select d.id,d.provinceid,d.cityid,d.title,d.address,d.lal from shm_city as c left join shm_dealer as d on c.yid = d.cityid" . $where;
