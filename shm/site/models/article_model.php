    ...
    */
    ...
    public function change_type_page($id, $cid)
    {
        ...
        if (!empty($data['one'])) {
            $sortId = $data['one']['sort_id'];
            //获取上一页的新闻
            $data['prev'] = $this->db->where(array('sort_id <' => $sortId, 'type_id' => $data['one']['type_id'], 'audit' => 1))->order_by("sort_id", "desc")->get('article')->row_array();
            //获取下一页的新闻
            $data['next'] = $this->db->where(array('sort_id >' => $sortId, 'type_id' => $data['one']['type_id'], 'audit' => 1))->order_by("sort_id", "asc")->get('article')->row_array();
            //返回上一页列表
            $data['back'] = $this->db->where(array('id' => $id, 'type_id' => $data['one']['type_id'], 'audit' => 1))->get('article')->row_array();
        }
        return $data;
    }
