<div class="btn-group">
    <a href="<?php echo site_urlc('recruit/create'); ?>" class='btn btn-primary'>
...
<?php if ($ctype = list_coltypes($this->cid, 0, 'ctype')) { ?>
    <form class="form-inline" action="<?php echo site_urlc($this->class . '/search/' . $this->cid); ?>" method="GET">
        <?php
        if (isset($_GET['type_id']) AND is_numeric($_GET['type_id']) AND $_GET['type_id']) {
            echo ui_btn_select('type_id', set_value("type_id", $_GET['type_id']), $ctype);
        } else {
            echo ui_btn_select('type_id', set_value("type_id"), $ctype);
        }
        ?>
        ...
        <table class="table table-striped table-hover select-list">
            ...
            <?php foreach ($list as $v): ?>
                <tr data-id="<?php echo $v['id'] ?>" data-sort="<?php echo $v['sort_id'] ?>">
                    ...
                    <td>
                        <a title='越大越前'>
                            <input type="text" class="sortid" value="<?php echo $v['sort_id'] ?>" data-id="<?php echo $v['id'] ?>">
                        </a>
                    </td>
                    ...
                    <td>
                        <div class="btn-group">
                            <?php if (ENVIRONMENT == "development") { ?>
                                <a class='btn  btn-small btn-ajax-copy' data-id="<?php echo $v['id'] ?>" href="#" title="复制">
                                    复制
                                </a>
                            <?php } ?>

                            <?php include 'inc_ui_audit.php'; ?>
                            <a class='btn btn-small' href=" <?php echo site_urlc($this->router->class . '/edit/' . $v['id']) ?> " title="<?php echo lang('edit') ?>">
                                <i class="fa fa-pencil"></i> <?php // echo lang('edit'); ?>
                            </a>
                            <a class='btn btn-danger btn-small btn-del' data-id="<?php echo $v['id'] ?>" href="#" title="<?php echo lang('del') ?>">
                                <i class="fa fa-times"></i> <?php // echo lang('del'); ?>
                            </a>
                        </div>
                    </td>
                </tr>
