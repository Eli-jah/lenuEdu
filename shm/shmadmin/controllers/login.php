    ...
    public function index()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->input->post('remember')) {
                ...
                $remember_hours = $this->mcfg->get('adminer', 'remember_hours');
                if (!is_numeric($remember_hours)) {
                    $remember_hours = 72;
                }

                $_remember = md5(HMACPWD . $info['uname'] . $session['login_ip']);
                $cookie = array(
                    'name' => '_remember',
                    'value' => $_remember,
                    'expire' => 60 * 60 * $remember_hours,
